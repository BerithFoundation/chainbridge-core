...
func KeypairFromAddress(addr, chainType, path, pswd string, insecure bool) (crypto.Keypair, error) {
	if insecure {
	...
	if pswd == "" {
		pswd = string(GetPassword(fmt.Sprintf("Enter password for key %s:", path)))
	}

	kp, err := ReadFromFileAndDecrypt(path, []byte(pswd), keyMapping[chainType])
	if err != nil {
		return nil, err
	}
